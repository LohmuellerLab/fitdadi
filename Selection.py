...
I added in multiprocessing capabilities because generating all the spectra takes SOOOO
long for large values of gamma. One solution is to not integrate where the gammas are
large, since the SFSes there should be close to 0, but I wonder if that will affect the
results when there are really large population size changes in the model... 

To do: PEP8 formatting
"""
...
#have user do pickling, should be easier that way since it's no longer mandatory
#try:
...
class spectra:
	def __init__(self, params, ns, demo_sel_func, pts=500, pts_l="empty", Npts=500, steps=False, n=20170., int_breaks=(0.000001, 0.00001, 0.0001, 0.001, 0.01, 1), int_bounds=(1e-4, 1000.), mp=False, echo=False, cpus=None):
                """
                params = optimized demographic parameters, don't include gamma ehre
                demo_sel_func = dadi demographic function with selection. gamma must be the last argument.
                ns = sample sizes
                pts = number of grid points over which to integrate
                steps = use this to set break points for spacing out the intervals
                mp= True if you want to use multiple cores (utilizes multiprocessing)
                if using the mp option you must also specify # of cpus, otherwise this will just use ncpus-1 on your machine
                """
		self.ns = ns
		self.spectra = []
                if steps:
                        ...
                        for i in reversed(range(0,numbreaks-1)):
                                self.gammas = numpy.append(self.gammas, -numpy.logspace(numpy.log10(int_breaks[i+1]), numpy.log10(int_breaks[i]), stepint))
                else:
                        self.gammas = -numpy.logspace(numpy.log10(int_bounds[1]), numpy.log10(int_bounds[0]), Npts)
                if pts_l == "empty":
                        self.pts = pts
		...
		func_ex = Numerics.make_extrap_log_func(demo_sel_func)
                ...
                if not mp:
                        for ii,gamma in enumerate(self.gammas):
                                self.spectra.append(func_ex(params+(gamma,), self.ns, self.pts_l))
                                if echo:
			...
			if cpus is None:
				cpus = multiprocessing.cpu_count() - 1
                        def worker_sfs(in_queue, outlist, popn_func_ex, params, ns, pts_l):
                                while True:
                                        ...
                                        if item == None:
                                                return
                                        ii, gamma = item
                                        sfs = popn_func_ex(params+(gamma,), ns, pts_l)
                                        print '{0}: {1}'.format(ii, gamma)
                                        result = (gamma, sfs)
                        ...
                        for i in xrange(cpus):
                                p = multiprocessing.Process(target=worker_sfs, args=(work, results, func_ex, params, self.ns, self.pts_l))
                                p.start()
                                pool.append(p)
                        for ii,gamma in enumerate(self.gammas):
                                work.put((ii, gamma))
	...
	def integrate(self, params, sel_dist, theta):
		"""
                integration without re-normalizing the DFE. This assumes the portion of the DFE you do not integrate is not seen in your sample. 
		"""
		#need to include tuple() here to make this function play nice with numpy arrays
		...
		if pops == 1:
			integrated = self.neu_spec*weight_neu + Numerics.trapz(weights[:,numpy.newaxis]*self.spectra, self.gammas, axis=0)
		elif pops == 2:
			integrated = self.neu_spec*weight_neu + Numerics.trapz(weights[:,numpy.newaxis,numpy.newaxis]*self.spectra, self.gammas, axis=0)
		elif pops == 3:
			integrated = self.neu_spec*weight_neu + Numerics.trapz(weights[:,numpy.newaxis,numpy.newaxis,numpy.newaxis]*self.spectra, self.gammas, axis=0)
		else:
			raise IndexError("Must have one to three populations")
		
		#integrated = scipy.integrate.simps(weights[:,numpy.newaxis]*spectra2, gammas, axis=0)
...
#code taken from dadi.Inference and modified for use with pre-generated arrays
#changed the objective function and optimization function to not use pts and instead use
#cached spectra
...
def optimize_cons(p0, data, model_func, sel_dist, theta, lower_bound=None, upper_bound=None,
                 verbose=0, flush_delay=0.5, epsilon=1e-4, constraint=None, 
    ...
    ####NEED TO DEFINE CONSFUNC and bnds ####
...
def optimize(p0, data, model_func, sel_dist, theta, lower_bound=None, upper_bound=None,
             verbose=0, flush_delay=0.5, epsilon=1e-3, 
             gtol=1e-5, multinom=False, maxiter=None, full_output=False,
             func_args=[], func_kwargs={}, fixed_params=None, ll_scale=1,
             output_file=None):
